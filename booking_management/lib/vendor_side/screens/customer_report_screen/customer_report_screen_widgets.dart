...
class CustomerReportSearchListModule extends StatelessWidget {
  CustomerReportSearchListModule({Key? key}) : super(key: key);
  final screenController = Get.find<CustomerReportScreenController>();

  @override
  Widget build(BuildContext context) {
    return screenController.customerReportSearchList.isEmpty
        ? const Center(child: Text("No data available!"))
        : ListView.builder(
      itemCount: screenController.customerReportSearchList.length,
      physics: const BouncingScrollPhysics(),
      shrinkWrap: true,
      itemBuilder: (context, i) {
        CustomerReportData singleItem = screenController.customerReportSearchList[i];
