...
class ScanAppointmentDetailsScreenController extends GetxController {
  /// "appointmentId" From Appointment List Screen
  String bookingId = Get.arguments;

  // int appointmentId = Get.arguments[1];
  ...
  getAppointmentDetailsByIdFunction() async {
    ...
    try {
      ...
      if (isSuccessStatus.value) {
        ...
        appointmentId.value = appointmentDetailsData.id!;
  ...
  confirmAppointmentByIdFunction() async {
    ...
    String url = ApiUrl.vendorAppointmentStatusChangeApi +
        "?status=Confirm&id=${appointmentId.value}";
    ...
    try {
      ...
      if (isSuccessStatus.value) {
        Fluttertoast.showToast(
            msg: "Appointment Confirmed!", toastLength: Toast.LENGTH_SHORT);
  ...
  doneAppointmentByIdFunction() async {
    isLoading(true);
    String url = ApiUrl.vendorAppointmentStatusChangeApi +
        "?status=Done&id=${appointmentId.value}";
    log("Appointment Status Change API URL : $url");
    ...
    try {
      ...
      if (isSuccessStatus.value) {
        Fluttertoast.showToast(
            msg: "Appointment Done!", toastLength: Toast.LENGTH_SHORT);
  ...
  cancelAppointmentByIdFunction() async {
    ...
    try {
      ...
      request.fields['BookingId'] = "${appointmentId.value}";
      ...
      response.stream
        ...
        if (isSuccessStatus.value) {
          ...
          Fluttertoast.showToast(
              msg: 'Successfully Cancelled', toastLength: Toast.LENGTH_SHORT);
